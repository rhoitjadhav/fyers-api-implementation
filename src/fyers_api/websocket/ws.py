...
class FyersSocket():
    ...
    def parse_symbol_data(self):
        ...
        for i in range(packet_length):
            (fyToken, timestamp, fyCode, fyFlag, pktLen) = self.FY_P_HEADER_FORMAT.unpack(
                self.message[:self.FY_P_LEN_HEADER])
            ...
            pc, ltp, op, hp, lp, cp, mop, mhp, mlp, mcp, mv = self.FY_P_COMMON_7208.unpack(
                self.message[: self.FY_P_LEN_COMN_PAYLOAD])
    ...
    def socket_data_define(self, symbol="", unsubscribe_flag=False):
        ...
        if self.data_type == "orderUpdate":
            ...
            get_quotes = requests.get(url=url,
                                      headers={"Authorization": self.access_token, 'Content-Type': self.content})
